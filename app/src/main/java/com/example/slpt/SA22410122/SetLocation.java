...
import com.example.slpt.SA22410122.CarSelection;
...
public class SetLocation extends AppCompatActivity {
    ...
    private void calculateDistance() {
        if (userLocation != null) {
            ...
            if (!origin.isEmpty() && !destination.isEmpty()) {
                ...
                distanceTextView.setText("Distance: " + latitude + " " + longitude);

                saveDataToFirebase(userLocation, origin, destination, distanceText);
            } else {
    ...
    private void saveDataToFirebase(Location userLocation, String origin, String destination, String distance) {
        if (userLocation != null) {
            String userLocationText = "Lat: " + userLocation.getLatitude() + ", Lon: " + userLocation.getLongitude();
            LocationData locationData = new LocationData(userLocationText, origin, destination, distance);
            String dataId = databaseReference.push().getKey();

            databaseReference.child("Passenger").child(FirebaseAuth.getInstance().getCurrentUser().getPhoneNumber()).child("locationData").child(dataId).setValue(locationData);
        }
    }
    ...
    public class LocationData {
        ...
        public LocationData(String userLocation, String origin, String destination, String distance) {
